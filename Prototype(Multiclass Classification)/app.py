...
def stdd(data):
    ...
    new_cat=pd.DataFrame(encoderr.transform([[df['proto'][0], df['service'][0], df['state'][0]
...
def predict():
    ...
    if(output=="Normal"):
        return render_template("home.html",prediction_text="Safe!!! Not an Intrusion {}".format(output))    
    return render_template("home.html",prediction_text="Alert!!! It is an Intrusion attack type is {}".format(output))
